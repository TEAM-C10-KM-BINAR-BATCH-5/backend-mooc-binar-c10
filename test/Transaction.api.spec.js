...
describe('API payment & transaction', () => {
  ...
  it('Failed enroll course because course already enrolled', async () => {
    const tokenUser = await getToken({
      email: 'syifa@gmail.com',
      password: 'usersyifa123',
    })
  ...
  it('Success get all transaction', async () => {
    ...
    const response = await request(app)
      ...
      .set('Authorization', `Bearer ${tokenAdmin}`)
    idTransaction = response.body.data[0].id
    expect(response.statusCode).toBe(200)
    expect(response.body.success).toBe(true)
    expect(response.body.message).toBe('Success, fetch')
  }, 10000)

  it('Success get transaction by id', async () => {
    const tokenUser = await getToken({
      email: 'syifa@gmail.com',
      password: 'usersyifa123',
    })
    const response = await request(app)
      .get(`/api/v1/user-transaction/${idTransaction}`)
      .set('Authorization', `Bearer ${tokenUser}`)
    expect(response.statusCode).toBe(200)
    expect(response.body.success).toBe(true)
    expect(response.body.message).toBe('Success, fetch')
  ...
  it('Failed get transaction by id because not found', async () => {
    const tokenUser = await getToken({
      email: 'syifa@gmail.com',
      password: 'usersyifa123',
    })
    const response = await request(app)
      .get('/api/v1/user-transaction/99')
      .set('Authorization', `Bearer ${tokenUser}`)
    expect(response.statusCode).toBe(404)
    expect(response.body.success).toBe(false)
    expect(response.body.message).toBe('id does not exist')
  ...
  it('Failed get transaction by id because user role is admin', async () => {
    const tokenAdmin = await getTokenAdmin({
  ...
  it('Failed get user transcaction because jwt malformed', async () => {
    const tokenMalformed =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9eyJpZCI6NywibmFtZSI6ImFkbWluIiiOiJiaW5hci50ZWFtLmMxMEBnbWFpbC5jb20iLCJyb2xl3MDI0NjA3NPeB0iP5KtK97mMc4jzq6poxj9c'
    const response = await request(app)
      .get(`/api/v1/user-transaction/${idTransaction}`)
  ...
  it('Failed get user transcaction by id because jwt expired', async () => {
    const tokenExpired =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6NywibmFtZSI6ImFkbWluIiwiZW1haWwiOiJiaW5hci50ZWFtLmMxMEBnbWFpbC5jb20iLCJyb2xlIjoiYWRtaW4iLCJpYXQiOjE3MDIzNzc1NzksImV4cCI6MTcwMjQ2Mzk3OX0.4eAhzrpoZ9kUnabNdil8YHxkVNa-EnD5iimahZ8ky2g'
    const response = await request(app)
      .get(`/api/v1/user-transaction/${idTransaction}`)
  ...
  it('User Failed get all User Transaction because user role is admin', async () => {
    const tokenAdmin = await getTokenAdmin({
      email: 'admin2@gmail.com',
      password: 'admin2123',
    })
  ...
  it('Failed route does not exist', async () => {
    ...
    const response = await request(app)
      .get('/api/v1/user-transaction/gege/10')
      .set('Authorization', `Bearer ${tokenUser}`)
    expect(response.statusCode).toBe(404)
    expect(response.body.success).toBe(false)
  ...
  it('Failed get user transaction by id', async () => {
    const tokenUser = await getToken({
      email: 'syifa@gmail.com',
      password: 'usersyifa123',
    })
    const response = await request(app)
      .get('/api/v1/user-transaction/1')
      .set('Authorization', `Bearer ${tokenUser}`)
    expect(response.statusCode).toBe(404)
    expect(response.body.success).toBe(false)
    expect(response.body.message).toBe('id does not exist')
